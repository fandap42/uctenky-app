...
export const TicketMobileList = memo(function TicketMobileList({ tickets, onTicketClick, viewSwitch }: TicketMobileListProps) {
  ...
  return (
    <div className="w-full space-y-4">
      ...
      <div className="flex flex-row items-start justify-between gap-4 pb-2">
        <div className="flex flex-wrap gap-2 w-full max-w-[280px]">
          {FILTERS.map((filter) => {
        ...
        {viewSwitch && (
          <div className="flex-shrink-0 mt-0 pt-0.5">
            {viewSwitch}
          </div>
        )}
      </div>
...
export const TicketCardItem = memo(function TicketCardItem({ ticket, onClick }: { ticket: Ticket; onClick: () => void }) {
  ...
  return (
    <Card
      onClick={onClick}
      className={cn(
        "p-4 relative overflow-hidden rounded-[1.5rem] border-border/50 shadow-sm w-full transition-all active:scale-[0.98] active:bg-muted/50 cursor-pointer hover:shadow-md",
        isUnpaidDone && "border-status-pending border-2",
        isReturned && "border-destructive border-2"
      )}
    >
      <div className="absolute top-1/2 -translate-y-1/2 right-0 p-3">
        <div className={cn("w-2.5 h-2.5 rounded-full shadow-sm ring-4 ring-background", statusConfig?.color || "bg-gray-400")} />
      </div>

      <div className="space-y-3 pr-6">
        {/* Row 1: Title + Amount */}
        <div className="flex justify-between items-start gap-2">
          <h4 className="font-bold text-sm leading-tight line-clamp-2 min-w-0 flex-1" title={ticket.purpose}>{ticket.purpose}</h4>
          <div className="flex flex-col items-end flex-shrink-0">
            <span className="text-[10px] text-muted-foreground">
              {new Date(ticket.targetDate).toLocaleDateString("cs-CZ")}
            </span>
            <span className="text-sm font-black text-foreground tabular-nums opacity-90">{displayAmount.toLocaleString("cs-CZ")} Kč</span>
          </div>
        </div>
