# 4FISuctenky - Uživatelská příručka

## Úvod

**4FISuctenky** je webová aplikace pro správu účtenek a finančních náhrad studentské organizace 4FIS. Tato příručka vás provede základním používáním aplikace.

---

## Pro členy organizace

### Přihlášení

1. Otevřete aplikaci v prohlížeči
2. Zadejte svůj **email** a **heslo**
3. Klikněte na **Přihlásit se**

### Přehled (Dashboard)

Po přihlášení uvidíte:
- **Počet vašich žádostí** - celkový počet podaných žádostí
- **Čeká na schválení** - žádosti čekající na vyřízení
- **Moje žádosti** - tabulka všech vašich žádostí

---

## Podávání žádosti o nákup

### Kdy podat žádost?

> ⚠️ **DŮLEŽITÉ:** Žádost musíte podat **minimálně 7 dní před plánovaným datem nákupu!**

### Jak podat žádost?

1. Klikněte na tlačítko **Nová žádost**
2. Vyplňte formulář:
   - **Účel nákupu** - stručný popis, co kupujete a proč
   - **Odhadovaná částka** - předpokládaná cena v Kč
   - **Plánované datum nákupu** - kdy chcete nákup provést
3. Klikněte na **Odeslat žádost**

### Stavy žádosti

| Stav | Popis |
|------|-------|
| 🟡 **Čeká na schválení** | Žádost čeká na schválení vedoucím |
| ✅ **Schváleno** | Můžete provést nákup |
| ❌ **Zamítnuto** | Žádost byla zamítnuta |
| 🔵 **Nakoupeno** | Nákup proveden, účtenka nahrána |
| ✔️ **Ověřeno** | Účtenka zkontrolována týmem Finance |

---

## Nahrání účtenky

### Po provedení nákupu:

1. V tabulce žádostí najděte schválenou žádost
2. Klikněte na **Nahrát účtenku**
3. Vyplňte:
   - **Obrázek účtenky** - fotografie nebo sken
   - **Obchod** - kde jste nakoupili
   - **Datum nákupu** - skutečné datum z účtenky
   - **Skutečná částka** - přesná částka z účtenky

### Požadavky na fotografii účtenky

> ⚠️ **KRITICKÉ:** Fotografie účtenky musí být **co nejkvalitnější!**

Na účtence **MUSÍ být čitelné:**

1. ✅ **Datum nákupu** - den, měsíc, rok
2. ✅ **Celková částka** - koncová suma
3. ✅ **Způsob platby** - musí být zřejmé, že nákup proběhl **v hotovosti**
4. ✅ **Název obchodu** - identifikace prodejce

### Tipy pro kvalitní fotografii:

- Fotografujte na **světlém pozadí**
- Zajistěte **dobré osvětlení** (bez stínů)
- Účtenka musí být **rovná a celá** ve snímku
- Zkontrolujte **ostrost** před odesláním
- Podporované formáty: **JPG, PNG, HEIC** (iPhone fotky)

---

## Pro vedoucí sekcí

### Přehled žádostí

Jako vedoucí sekce máte přístup k přehledu všech žádostí vaší sekce. Administrátoři mají navíc přístup ke všem sekcím najednou:

1. V menu klikněte na **Žádosti sekce**
2. Vedoucí sekce uvidí seznam všech žádostí od členů své sekce
3. **Administrátoři** mohou v horní části stránky přepínat mezi jednotlivými sekcemi pomocí výběrového pole
4. Můžete sledovat stav jednotlivých žádostí

> ⚠️ **Poznámka:** Schvalování žádostí provádí **pouze tým Finance**. Vedoucí sekce má přístup pouze pro čtení. Administrátoři mohou žádosti schvalovat přímo v tomto přehledu.

### Co můžete sledovat?

- Stav žádostí (čeká, schváleno, nakoupeno, ověřeno)
- Účel a odhadovanou částku
- Datum plánovaného nákupu

---

## Časté dotazy (FAQ)

### Proč musím žádost podat týden dopředu?
Abychom měli čas na schválení a přípravu hotovosti.

### Co když je skutečná částka jiná než odhadovaná?
To je v pořádku - při nahrání účtenky zadáte skutečnou částku.

### Mohu platit kartou?
**NE.** Všechny nákupy musí být provedeny **výhradně v hotovosti**.

### Jaký formát obrázku mohu nahrát?
Podporujeme **JPG, PNG** a **HEIC** (iPhone fotky - automaticky se konvertují).

### Co když je účtenka nečitelná?
Tým Finance vás kontaktuje a požádá o novou fotografii.

---

## Kontakt a podpora

V případě problémů kontaktujte tým Finance.

---

*4FISuctenky © 2026*
